    ...
    <div className="flex flex-1 overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto p-4 sm:p-6 md:p-8 no-scrollbar">
        <div className="max-w-[900px] mx-auto p-4 md:p-6 lg:p-8 pt-8">
          ...
          <div className="mb-8 border-b-4 border-gray-200 dark:border-white/20 pb-6 flex items-center justify-between">
            <div>
            ...
            <div className="w-16 h-16 rounded-2xl bg-[#ff90e8] border border-gray-200 dark:border-white/20 shadow-sm flex items-center justify-center font-bold text-2xl">
              {likedPosts.length}
            </div>
          </div>
          ...
          <div className="w-full mb-8 rounded-2xl overflow-hidden shadow-sm border border-gray-100 dark:border-white/10 bg-white dark:bg-[#0f172a] relative h-32 group cursor-pointer flex items-center justify-center">
            ...
            <div className="relative z-10 w-full px-6 flex flex-col justify-center h-full text-white">
              ...
              <div>
                <button className="text-xs bg-white dark:bg-[#0f172a] text-gray-900 dark:text-white px-5 py-2 rounded-lg font-bold hover:bg-[#ff90e8] hover:text-white transition-colors shadow-sm border border-transparent hover:border-white/20">Learn More</button>
              </div>
            </div>
          </div>

          {likedPosts.length === 0 ? (
            <div className="bg-white dark:bg-[#0f172a] rounded-3xl border border-gray-200 dark:border-white/20 shadow-md p-12 text-center flex flex-col items-center">
              <ThumbsUp size={48} className="text-gray-300 mb-4" />
              <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">No liked threads yet</h3>
              <p className="text-gray-500 dark:text-gray-400 font-medium">
                Go back to the feed and click the Like button on posts you find interesting!
              </p>
              <Link href="/platform" className="mt-6 bg-[#5a32fa] text-white px-6 py-3 rounded-xl font-bold text-sm border border-gray-200 dark:border-white/20 hover:shadow-sm hover:-translate-y-1 transition-all">
                Return to Feed
              </Link>
            </div>
          ) : (
            <div className="space-y-6 pb-24">
              {likedPosts.map((post) => {
                ...
                return (
                  <div key={post.id} className="bg-white dark:bg-[#0f172a] rounded-3xl border border-gray-200 dark:border-white/20 shadow-md overflow-hidden">
                    {/* Post Header */}
                    <div className="p-6 pb-4 flex items-start justify-between border-b-2 border-gray-100 dark:border-white/10">
                      <div className="flex items-center gap-3">
                        <div 
                          className="w-12 h-12 rounded-full text-[#131313] flex items-center justify-center text-xl font-bold border border-gray-200 dark:border-white/20"
                          style={{ backgroundColor: post.color }}
                        >
                          {post.initial}
                        </div>
                        <div>
                          <h3 className="font-bold text-gray-900 dark:text-white text-[15px] hover:underline cursor-pointer">{post.name}</h3>
                          <p className="text-[13px] text-gray-500 dark:text-gray-400 font-medium">{post.title}</p>
                          <p className="text-[11px] text-gray-400 font-bold mt-0.5">{post.time}</p>
                        </div>
                      </div>
                      <button className="text-gray-400 hover:text-gray-900 dark:text-white transition-colors p-2 hover:bg-gray-100 dark:bg-white/10 rounded-full">
                        <MoreHorizontal size={20} />
                      </button>
                    </div>
      
                    {/* Post Content */}
                    <div className="p-6 pt-4 text-gray-800 dark:text-gray-100 text-[15px] leading-relaxed">
                      {post.content}
                    </div>
      
                    {/* Post Actions */}
                    <div className="px-4 py-3 bg-gray-50 dark:bg-white/5 flex items-center justify-between border-t-2 border-gray-200 dark:border-white/20">
                      <div className="flex gap-2">
                        ...
                        <button className="flex items-center gap-2 px-4 py-2 rounded-xl font-bold text-sm text-gray-600 dark:text-gray-300 transition-all hover:bg-gray-200 border-2 border-transparent hover:border-gray-300">
                          <MessageCircle size={18} /> Comment {post.comments > 0 && `(${post.comments})`}
                        </button>
                      </div>
                      <div className="flex items-center gap-2 px-4 py-2 font-bold text-sm text-gray-500 dark:text-gray-400">
