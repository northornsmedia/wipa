...
export default function ResearchReportsHubPage() {
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] dark:bg-[#070b14] text-slate-900 dark:text-slate-100 font-sans selection:bg-red-500/20 pb-28 overflow-x-hidden">
      
      {/* Top Hero Banner & Institutional Header - Big, Smooth, Cool & Fluid */}
      <div className="relative bg-white dark:bg-[#090d16] border-b border-slate-200 dark:border-white/10 pt-12 pb-14 md:pt-16 md:pb-18 shadow-xs overflow-hidden">
        {/* Ambient Subtle Red Backdrop Glow */}
        <div className="absolute -top-24 left-1/4 w-[600px] h-[350px] bg-red-500/10 dark:bg-red-600/15 rounded-full blur-[140px] pointer-events-none" />
        <div className="absolute -bottom-20 right-1/4 w-[400px] h-[250px] bg-rose-500/10 dark:bg-rose-600/10 rounded-full blur-[120px] pointer-events-none" />
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
            <div>
              <div className="flex items-center gap-2 mb-3">
                <span className="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full bg-red-500/10 border border-red-500/20 text-red-600 dark:text-red-400 text-[11px] font-black uppercase tracking-wider">
              ...
              <h1 className="text-4xl sm:text-6xl md:text-7xl lg:text-8xl font-black tracking-tight uppercase text-slate-900 dark:text-white leading-[1.05]">
                Research <span className="bg-gradient-to-r from-red-600 via-rose-500 to-red-500 bg-clip-text text-transparent">& Reports</span>
              </h1>
              <p className="mt-3 text-sm sm:text-base md:text-lg font-medium text-slate-600 dark:text-slate-400 max-w-3xl leading-relaxed">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
              {/* Search Box */}
              <div className="relative flex-1 sm:w-80 flex items-center border border-slate-200 dark:border-white/15 bg-slate-50 dark:bg-slate-900/90 rounded-2xl px-3.5 py-2.5 shadow-2xs focus-within:border-red-500 focus-within:ring-2 focus-within:ring-red-500/15 transition-all">
              ...
              <Link
                ...
                className="inline-flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 active:scale-95 text-white px-5 py-3 text-xs font-black uppercase tracking-wider shadow-md shadow-red-500/25 transition-all shrink-0 cursor-pointer"
              >
                <Upload size={14} />
                ...
                <button
                  ...
                  className={`pb-3.5 px-3 text-xs sm:text-sm font-black uppercase tracking-wider transition-all whitespace-nowrap relative flex items-center gap-2 cursor-pointer ${
                    isActive 
                      ...
                      : (isLexis ? 'text-red-600/70 hover:text-red-600 dark:text-red-400/80' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white')
                  }`}
                >
                  {isLexis && <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse" />}
                  <span>{sub.name}</span>
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-mono font-bold transition-all ${
                    isActive 
                      ? 'bg-red-500/15 text-red-600 dark:text-red-300 font-black' 
                  ...
                  {isActive && (
                    <span className="absolute bottom-0 left-0 right-0 h-1 rounded-t-full bg-red-600 shadow-sm shadow-red-500/50" />
                  )}
                </button>
              );
            })}
      ...
      {/* Main Fluid Content Canvas - Unboxed & Dynamic */}
      <div className="w-full px-4 sm:px-8 md:px-12 lg:px-16 pt-10">
          ...
          <div className="mb-14">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xs font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse"></span> Featured Publication
              </h2>
            </div>

            <div className="relative overflow-hidden rounded-3xl border border-slate-200/90 dark:border-white/10 bg-white dark:bg-[#0d1322] shadow-xl p-6 sm:p-10 lg:p-12 transition-all">
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
                
                {/* Clean Publication Dossier Cover Preview */}
                <div className="lg:col-span-5 relative group">
                  <div className="absolute -inset-2 bg-gradient-to-tr from-red-500/20 via-rose-500/15 to-transparent rounded-3xl blur-2xl opacity-40 group-hover:opacity-70 transition duration-700"></div>
                  
                  <div className="relative aspect-[3/4] max-h-[440px] mx-auto rounded-2xl overflow-hidden border-2 border-slate-900/80 dark:border-white/10 shadow-2xl shadow-black/40 group-hover:-translate-y-1.5 transition-all duration-500 bg-slate-950">
                    <img 
                    ...
                    <div className="absolute top-4 left-4 flex items-center gap-2">
                      <span className="bg-red-600 text-white text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-md shadow-md">
                        {featuredItem.type}
                      </span>
                    </div>

                    {/* Clean Bottom Overlay Bar without duplicate text */}
                    <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between">
                      <span className="text-[10px] font-black uppercase tracking-wider text-white/95 bg-black/70 backdrop-blur-md px-2.5 py-1 rounded-md flex items-center gap-1.5">
                        <ShieldCheck size={12} className="text-red-400" />
                        <span>Verified Dossier</span>
                      </span>

                      <span className="text-[10px] font-mono font-bold text-white/95 bg-black/70 backdrop-blur-md px-2.5 py-1 rounded-md">
                        {featuredItem.format || featuredItem.time}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="lg:col-span-7 flex flex-col justify-between space-y-6">
                  ...
                  <div className="flex flex-wrap items-center gap-4 pt-2">
                    <a
                      ...
                      className="inline-flex items-center justify-center gap-2.5 bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 text-white font-black text-xs uppercase tracking-widest px-8 py-4 rounded-2xl shadow-lg shadow-red-500/25 transition-all hover:scale-105 active:scale-95 cursor-pointer"
                    >
                      <Download size={16} />
                    ...
                    <Link
                      ...
                      className="inline-flex items-center justify-center gap-2 bg-slate-100 hover:bg-slate-200 dark:bg-white/5 dark:hover:bg-white/10 hover:text-red-600 dark:hover:text-red-400 text-slate-800 dark:text-slate-200 font-bold text-xs uppercase tracking-wider px-6 py-4 rounded-2xl border border-slate-200 dark:border-white/10 transition-all cursor-pointer"
                    >
                      <BookOpen size={14} />
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-white/10 pb-4">
            <div>
              <h2 className="text-xl font-black uppercase tracking-tight text-slate-900 dark:text-white flex items-center gap-2">
                <FileCheck size={20} className="text-red-500" /> Database Archive & Publications
              </h2>
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
              ...
              <div className="relative">
                <select 
                  ...
                  className="appearance-none bg-white dark:bg-[#0d1322] border border-slate-200 dark:border-white/10 rounded-xl px-4 py-2 pr-9 text-xs font-bold text-slate-700 dark:text-slate-200 focus:outline-none focus:border-red-500 cursor-pointer shadow-2xs"
                >
              ...
              <div className="flex items-center p-1 rounded-xl bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10">
                <button
                  ...
                  className={`p-1.5 rounded-lg transition-all cursor-pointer ${
                    viewMode === 'grid' ? 'bg-white dark:bg-slate-800 text-red-600 shadow-2xs' : 'text-slate-400 hover:text-slate-700 dark:hover:text-white'
                  }`}
                ...
                <button
                  ...
                  className={`p-1.5 rounded-lg transition-all cursor-pointer ${
                    viewMode === 'table' ? 'bg-white dark:bg-slate-800 text-red-600 shadow-2xs' : 'text-slate-400 hover:text-slate-700 dark:hover:text-white'
                  }`}
          ...
          {loading ? (
            ...
            <div className="p-16 text-center rounded-3xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#0d1322]">
              ...
              <button
                ...
                className="mt-4 px-4 py-2 rounded-xl bg-red-600 text-white text-xs font-bold hover:bg-red-700 transition cursor-pointer"
              >
                Reset Filters
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredResources.map((item) => (
                <div 
                  key={item.id}
                  className="group flex flex-col justify-between rounded-3xl border border-slate-200/90 dark:border-white/10 bg-white dark:bg-[#0d1322] overflow-hidden shadow-2xs hover:shadow-xl transition-all duration-300 hover:border-red-500/40 hover:-translate-y-1"
                >
                  <div>
                    {/* Cover Preview Image */}
                    <div className="relative aspect-[16/10] w-full overflow-hidden bg-slate-900">
                      ...
                      <div className="absolute top-3 left-3 flex items-center gap-2">
                        <span className="bg-red-600 text-white text-[9px] font-black uppercase tracking-wider px-2.5 py-0.5 rounded-md shadow-sm">
                          {item.type}
                        </span>
                    ...
                    {/* Card Body */}
                    <div className="p-6">
                      <h3 className="text-base sm:text-lg font-black leading-snug tracking-tight text-slate-900 dark:text-white group-hover:text-red-600 transition-colors mb-2 line-clamp-2">
                        {item.title}
                      </h3>
                      ...
                      <div className="text-xs font-semibold text-slate-600 dark:text-slate-300 flex items-center gap-1.5 pt-1">
                        <Building size={13} className="text-red-500 shrink-0" />
                  ...
                  <div className="p-4 px-6 border-t border-slate-100 dark:border-white/5 bg-slate-50/60 dark:bg-slate-900/40 flex items-center justify-between gap-3">
                    <Link
                      ...
                      className="text-xs font-bold text-slate-600 dark:text-slate-300 hover:text-red-600 transition-colors inline-flex items-center gap-1"
                    >
                      <span>Abstract</span>
                    ...
                    <a
                      ...
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-900 hover:bg-red-600 text-white dark:bg-white dark:text-slate-900 dark:hover:bg-red-600 dark:hover:text-white text-xs font-bold transition-all shadow-xs active:scale-95"
                    >
                      <Download size={12} />
                      <span>Download</span>
                    </a>
                      ...
                      <tr key={item.id} className="hover:bg-slate-50/80 dark:hover:bg-white/[0.02] transition-colors group">
                        ...
                        <td className="py-4 px-6">
                          <Link href={`/platform/resources/research-reports/${item.id}`} className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <span className="font-bold text-slate-900 dark:text-white group-hover:text-red-600 transition-colors block text-sm line-clamp-1">
                                {item.title}
                              </span>
                            </div>
                          </Link>
                        ...
                        <td className="py-4 px-6 text-right">
                          <a
                            ...
                            target={item.download_url?.startsWith('http') ? '_blank' : '_self'}
                            rel="noopener noreferrer"
                            className="inline-flex items-center justify-center w-8 h-8 rounded-full border border-slate-200 dark:border-white/10 text-slate-500 dark:text-slate-300 hover:bg-red-600 hover:border-red-600 hover:text-white transition-all shadow-2xs"
