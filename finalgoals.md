...
- [x] 10. `WIPA/src/app/signup/page.tsx` — Wire signup form to `supabase.auth.signUp()`, passing `full_name` in options metadata.
- [x] 11. Implement real-time password strength meter and email format validation in signup UI.
- [x] 12. Add Google OAuth and Microsoft OAuth buttons calling `supabase.auth.signInWithOAuth()`.
- [x] 13. `WIPA/src/app/login/page.tsx` — Wire login form to `supabase.auth.signInWithPassword()`, storing session and redirecting to `/platform`.
- [x] 14. Wire "Forgot Password" modal calling `supabase.auth.resetPasswordForEmail()`.
- [x] 15. Create `WIPA/src/app/auth/callback/route.ts` to handle OAuth code exchange and session confirmation.
- [x] 16. Wire Logout action in `PlatformHeader` and `Sidebar` calling `supabase.auth.signOut()` and clearing Zustand state.

### 2.2 — Multi-Step Onboarding Flow (`WIPA`)
- [x] 17. `WIPA/src/app/onboarding/page.tsx` — Implement 4-step wizard:
...
- [x] 18. Save all onboarding data to `profiles` table and set `onboarding_completed: true`.
- [x] 19. Redirect to `/platform` with confetti celebration and award 100 Onboarding Welcome XP.
...
- [x] 20. `WIPA/src/app/platform/profile/page.tsx` — Fetch full profile data from Supabase `profiles` table for current logged-in user.
- [x] 21. Real-time stats display: Live connection count from `connections`, post count from `feed_posts`, forum replies count, and total XP badge.
- [x] 22. Wire "Edit Profile" modal to update `bio`, `title`, `company`, `practice_area`, `country`, `linkedin_url`, `website_url`.
- [x] 23. Wire Avatar and Cover Photo uploaders directly updating Supabase Storage and `profiles` URLs.
- [x] 24. Display verification status banner: "Pending Verification" (yellow), "Verified IP Practitioner" (blue shield), or "Upload Credentials" (gray).

### 3.2 — Public & Member Profile Pages (`WIPA`)
- [x] 25. `WIPA/src/app/platform/profile/[id]/page.tsx` — Dynamic profile view for any authenticated member.
- [x] 26. `WIPA/src/app/u/[member_id]/page.tsx` — Public shareable vanity profile (no authentication required) with SEO OpenGraph tags.
- [x] 27. Connect Button logic: Query `connections` table to determine state: "Connect", "Pending Request", "Accept Request", or "Connected".
- [x] 28. "Message" button routing directly to 1-on-1 thread with target user in `/platform/messages`.

### 3.3 — "Recommended by WIPA" Badge System
- [x] 29. Public Profile Badge: If `is_wipa_recommended === true`, render golden glowing badge with tooltip: "Recommended by WIPA since [Date]".
- [x] 30. Member Directory Badge: Show gold star indicator on member card in `/platform/members` and `/platform/network`.
- [x] 31. Feed Posts & Comments Badge: Display inline `⭐ WIPA` badge next to author name on all posts and forum comments.
- [x] 32. Notification on Grant: Insert notification to `notifications` table when admin grants badge: "Congratulations! You've been awarded the 'Recommended by WIPA' badge 🌟".
...
- [x] 33. Schema `feed_posts`: `id, author_id, content, media_urls TEXT[], privacy, likes_count, comments_count, is_pinned, created_at, updated_at`.
- [x] 34. `WIPA/src/app/platform/page.tsx` — Fetch paginated feed posts joined with author `profiles` (name, avatar, title, `is_wipa_recommended`, verification status).
- [x] 35. Post Creation Modal: Text editor, image/video attachment uploader (saving to `feed-media` bucket), privacy selector.
- [x] 36. Real-time Post Likes: `feed_likes` table with optimistic UI updates and Supabase DB trigger syncing `likes_count`.
- [x] 37. Threaded Post Comments: `feed_comments` table with instant insertion, author joining, and real-time count increments.
- [x] 38. Post Author Actions: Edit post modal and Delete post with confirmation (enforced by RLS `author_id === auth.uid()`).
- [x] 39. Supabase Realtime Subscription: Subscribe to `INSERT` on `feed_posts` to dynamically prepend new posts from other users.

### 4.2 — Discussion Forums (`WIPA`)
- [x] 40. Schema `forums`: `id, title, slug, description, icon, category, topics_count, posts_count, created_at`.
- [x] 41. Schema `forum_posts`: `id, forum_id, author_id, title, content, is_pinned, is_locked, views_count, likes_count, replies_count, created_at`.
- [x] 42. Schema `forum_replies`: `id, post_id, author_id, content, parent_reply_id, likes_count, created_at`.
- [x] 43. `WIPA/src/app/platform/forums/page.tsx` — Categories overview with real-time topic counts and active discussions.
- [x] 44. `WIPA/src/app/platform/forums/[forumId]/page.tsx` — Category thread listing with filter by Latest, Most Popular, and Unanswered.
- [x] 45. `WIPA/src/app/platform/forums/[forumId]/[postId]/page.tsx` — Full discussion post with rich text, author profile card, and threaded replies.
- [x] 46. "Create Topic" modal in forum categories with category selector, tag picker, and rich-text content input.
- [x] 47. "Liked Threads" page (`WIPA/src/app/platform/liked-threads/page.tsx`) listing all forum threads bookmarked by the user.
...
- [x] 48. Schema `conversations`: `id, type ('direct' | 'group'), title, created_at, updated_at`.
- [x] 49. Schema `conversation_participants`: `conversation_id, profile_id, last_read_at, unread_count`.
- [x] 50. Schema `messages`: `id, conversation_id, sender_id, content, attachments JSONB, is_read, created_at`.
- [x] 51. `WIPA/src/app/platform/messages/page.tsx` — Split-view chat interface:
...
- [x] 52. Real-time Chat Engine: Supabase Realtime channel subscription on `messages` filtered by active `conversation_id`.
- [x] 53. Message Sending & Delivery: Instant optimistic bubble rendering, server insert, updating conversation `updated_at`.
- [x] 54. Auto Read Receipts: Update `last_read_at` and `is_read = true` when conversation is focused.
- [x] 55. Sidebar live unread counter: Supabase listener updating total unread messages badge across all conversations.
- [x] 56. Community Channels (`#general`, `#daily-highlights`, `#time-tracking`, `#productivity-systems`) integration.

### 5.2 — Member Directory & Networking (`WIPA`)
- [x] 57. Schema `connections`: `id, requester_id, recipient_id, status ('pending' | 'accepted' | 'rejected'), created_at, updated_at`.
- [x] 58. `WIPA/src/app/platform/network/page.tsx` — Searchable and filterable grid of all WIPA members.
- [x] 59. Filter Bar: Filter by Country, Practice Area (Patents, Trademarks, Copyright, Litigation, Licensing), Industry Sector, and "WIPA Recommended" toggle.
- [x] 60. Member Cards: Avatar, Name, Title, Company, Country flag, Specialization chips, WIPA Recommended gold badge, "Connect" action.
- [x] 61. Connection Request Actions: "Send Request", "Cancel Request", "Accept Request", "Reject Request", "Remove Connection".
- [x] 62. "My Network" Tab: Dedicated view of currently accepted connections with direct messaging shortcuts.
...
- [x] 63. Schema `events`: `id, title, slug, description, event_date TIMESTAMPTZ, end_date TIMESTAMPTZ, timezone, location, is_virtual, meeting_url, recording_url, cover_image_url, organizer_id, category, price NUMERIC DEFAULT 0, max_attendees, current_attendees INTEGER DEFAULT 0, is_featured BOOLEAN DEFAULT false, is_published BOOLEAN DEFAULT true, created_at`.
- [x] 64. Schema `event_registrations`: `id, event_id, user_id, status ('registered' | 'attended' | 'cancelled'), ticket_type, created_at`.
- [x] 65. `WIPA/src/app/platform/events/page.tsx` — Events listing with Upcoming, Past, and Featured tabs, category filters, and search.
- [x] 66. `WIPA/src/app/platform/events/[id]/page.tsx` — Event deep-dive: Cover banner, countdown timer, speaker lineup, full agenda, location/platform details, RSVP button.
- [x] 67. One-Click RSVP: Insert to `event_registrations`, increment `current_attendees`, generate calendar invite.
- [x] 68. Calendar Sync Integration:
...
- [x] 69. `WIPA/src/app/platform/calendar/page.tsx` — Interactive monthly/weekly calendar view of all community events with personal registration highlights.
- [x] 70. Virtual Room Integration: Direct Meetn/Zoom room launch (`/api/meetn/create-room`) for authorized attendees.
...
- [x] 72. `WIPA/src/app/platform/resources/page.tsx` — Master resource directory index featuring cards for all 11 content verticals.
- [x] 73. `WIPA/src/components/Sidebar.tsx` — Collapsible animated Resource Library menu with dedicated ChevronDown toggle and zero page-jump.

### 7.2 — Resource Verticals Implementation (`WIPA`)
- [x] 74. **Webinars & Learning** (`/platform/resources/webinars`) — Video webinar archive with embedded video player, slide downloads, speaker bios, and transcript accordion.
- [x] 75. **Education & Dev** (`/platform/resources/education`) — University and executive IP courses directory, filterable by institution, degree level, duration, and topic.
- [x] 76. **Women's IP World** (`/platform/resources/womens-ip-world`) — Annual digital publication reader, spotlight articles, interviews with leading female practitioners, and downloadable PDF issues.
- [x] 77. **Articles & Insights** (`/platform/resources/articles-insights`) — Expert written analysis on patent litigation, trademark strategy, trade secrets, and emerging IP trends.
- [x] 78. **IP News & Legal Updates** (`/platform/resources/ip-news`) — Curated global intellectual property news feed categorized by jurisdiction (US PTO, EPO, WIPO, Asia-Pacific, Latin America).
- [x] 79. **Research & Reports** (`/platform/resources/research-reports`) — Comprehensive industry whitepapers, statistical IP benchmark reports, and downloadable PDF research.
- [x] 80. **Guides & Toolkits** (`/platform/resources/guides-toolkits`) — Practical checklists, contract templates, IP audit worksheets, and filing toolkits with one-click downloads.
- [x] 81. **Career & Leadership** (`/platform/resources/career-leadership`) — Leadership development guides, salary insights, career transition advice, and executive interview series.
- [x] 82. **In-House Counsel** (`/platform/resources/in-house-counsel`) — Dedicated hub for corporate IP directors, portfolio management frameworks, outside counsel budgeting, and risk mitigation strategies.
- [x] 83. **Podcasts & Conversations** (`/platform/resources/podcasts-conversations`) — Audio podcast player with playlist support, episode timestamps, host details, and audio waveform visualizer.
- [x] 84. **Wellness & Wellbeing** (`/platform/resources/wellness-v2`) — Mental health, work-life balance, mindfulness workshops, stress management for IP professionals, and retreat listings.

### 7.3 — Splash Sponsored Banner Integration (`WIPA`)
- [x] 85. `WIPA/src/components/SplashSponsoredBanner.tsx` — Full-width animated banner component rendering at top of sponsored resource categories:
...
- [x] 87. Schema `ip_firm_team_members`: `id, firm_id, profile_id UUID REFERENCES profiles(id), role, is_primary_contact`.
- [x] 88. Schema `ip_firm_reviews`: `id, firm_id, reviewer_id UUID REFERENCES profiles(id), rating INTEGER CHECK (rating BETWEEN 1 AND 5), review_text, created_at`.
- [x] 89. `WIPA/src/app/platform/resources/ip-firms/page.tsx` — Searchable firm directory:
...
- [x] 90. `WIPA/src/app/platform/resources/ip-firms/[slug]/page.tsx` — Comprehensive firm profile:
...
- [x] 91. `WIPA/src/app/platform/resources/ip-firms/claim/page.tsx` — Firm Claiming Wizard:
...
- [x] 93. Schema `business_team_members`: `id, business_id UUID REFERENCES business_profiles(id), profile_id UUID REFERENCES profiles(id), role, is_admin, joined_at`.
- [x] 94. `WIPA/src/app/platform/business/create/page.tsx` — 5-Step Creation Wizard:
...
- [x] 95. `WIPA/src/app/platform/business/[slug]/page.tsx` — Public Business Profile page showcasing team, jobs posted, articles authored, and services provided.
- [x] 96. `WIPA/src/app/platform/business/[slug]/edit/page.tsx` — Business settings dashboard for authorized business team members.
...
- [x] 98. Schema `job_applications`: `id, job_id UUID REFERENCES jobs(id), applicant_id UUID REFERENCES profiles(id), resume_url, cover_letter, status ('submitted' | 'reviewing' | 'interview' | 'rejected' | 'accepted'), created_at`.
- [x] 99. `WIPA/src/app/platform/jobs/page.tsx` — Interactive Jobs Board:
...
- [x] 100. `WIPA/src/app/platform/jobs/[id]/page.tsx` — Full job description view with quick-apply modal (resume upload, profile data pre-fill).
...
- [x] 101. Schema `quizzes`: `id, title, slug, category, difficulty ('Beginner' | 'Intermediate' | 'Advanced'), description, xp_reward, time_limit_seconds, passing_score, is_published, created_at`.
- [x] 102. Schema `quiz_questions`: `id, quiz_id UUID REFERENCES quizzes(id), question_text, explanation, order_num`.
- [x] 103. Schema `quiz_options`: `id, question_id UUID REFERENCES quiz_questions(id), option_text, is_correct, order_num`.
- [x] 104. Schema `quiz_attempts`: `id, quiz_id, user_id, score, total_questions, xp_earned, passed, answers JSONB, completed_at`.
- [x] 105. Schema `xp_history`: `id, user_id UUID REFERENCES profiles(id), action_type, xp_amount, reference_id, created_at`.
- [x] 106. `WIPA/src/app/platform/quizzes/page.tsx` — Quiz catalog sorted by difficulty and IP practice area.
- [x] 107. `WIPA/src/app/platform/quizzes/[id]/page.tsx` — Gamified test interface with timer, progress bar, animated option selection, and instant score celebration.
- [x] 108. XP Awarding API (`/api/xp/award`): Secure endpoint verifying quiz completion and updating user's `total_xp` and `xp_level`.
- [x] 109. `WIPA/src/app/platform/leaderboard/page.tsx` — Community leaderboard ranking top practitioners by weekly, monthly, and all-time XP with celebratory podium animations.
...
- [x] 110. Multi-Model Selection in `WIPA/src/components/LexIQChatCard.tsx`:
...
- [x] 111. System Prompt & Knowledge Base (`WIPA/src/lib/lexiq/system-prompt.ts`):
...
- [x] 112. Automated Navigation Execution: `LexIQChatCard.tsx` parses JSON navigation blocks and triggers instantaneous SPA routing via Next.js `useRouter().push()`.
- [x] 113. Thought Process Visualizer: Expandable dropdown in chat bubble rendering `reasoning_details` for reasoning-capable models.
- [x] 114. Seamless Typing UX: Automatic textarea refocusing when AI finishes generating response.
