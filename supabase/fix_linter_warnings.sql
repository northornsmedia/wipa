...
-- 2. HARDEN CONTENT & RESOURCE TABLES RLS
DO $$
DECLARE
  tbl TEXT;
...
-- 3. HARDEN ADS, ANALYTICS & SPECIFIC APP TABLES
DO $$
BEGIN
  -- ad_campaigns
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'ip_firm_reviews') THEN
    ...
    CREATE POLICY "Users can insert their own review" ON public.ip_firm_reviews FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
    CREATE POLICY "Users can update their own review" ON public.ip_firm_reviews FOR UPDATE TO authenticated USING (auth.uid() = user_id OR public.is_admin()) WITH CHECK (auth.uid() = user_id OR public.is_admin());
    CREATE POLICY "Users can delete their own review" ON public.ip_firm_reviews FOR DELETE TO authenticated USING (auth.uid() = user_id OR public.is_admin());
  END IF;
END $$;
